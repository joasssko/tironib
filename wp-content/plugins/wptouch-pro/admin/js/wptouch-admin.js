...
/*  New Page Menu Tabs */
	jQuery( function() {
		...
		jQuery( '#menu-select li a' ).bind( 'click', function() {
	...
	if ( jQuery( '#wptouch-icon-list' ).length ) {
		...
		jQuery( 'select#developer_mode' ).change( function() {
			switch( jQuery( this ).val() ) {
				case 'off':
					jQuery( '#setting_developer_mode_device_class' ).hide();
					break;	
				default:
					jQuery( '#setting_developer_mode_device_class' ).fadeIn();
					break;
			}
		});
		
		jQuery( 'select#developer_mode' ).change();
	...
	if ( wptouchPostIconType.length ) {
		wptouchPostIconType.change( function() {
			var selectedValue = wptouchPostIconType.val();		
	...
	if ( wptouchCalColor.length ) {
		wptouchCalColor.change( function() {
			var selectedValue = wptouchCalColor.val();		
	...
	if ( wptouchCalCustomColor.length ) {
		wptouchCalCustomColor.change( function() {
			...
			var selectedValue = wptouchCalCustomColor.val();		
	...
	var wptouchPageRedirectDropdown = jQuery( '#home_page_redirect_target' );
	if ( wptouchPageRedirectDropdown.length ) {
		wptouchPageRedirectDropdown.change( function() {
			if ( wptouchPageRedirectDropdown.val() == "custom" ) {
				jQuery( '#setting_home_page_redirect_custom' ).fadeIn();	
			} else {
				jQuery( '#setting_home_page_redirect_custom' ).hide();
			}
		});
		
		wptouchPageRedirectDropdown.change();	
	}	


/* Some EXPERIMENTAL code for keeping the icon pool in view */
//  jQuery( function(){
//  	var placeholder = jQuery( '#bnc .pool-wrapper' );
//  	var message = jQuery( '#wptouch-icon-packs' );
...
function WPtouchTooltipSetup() {
	...
	// Admin Tooltips	
	jQuery( 'a.wptouch-tooltip' ).tooltip( { effect: 'fade', position: 'top right', offset: [-12, -10], tip: '#wptouch-tooltip' });	
	
	// Admin Tooltips for items on the far right of the page
	jQuery( 'a.wptouch-tooltip-left' ).tooltip( { effect: 'fade', position: 'top left', offset: [-10, 7], tip: '#wptouch-tooltip-left' });
...
function WPtouchSetupThemeSettings() {	
...
function WPtouchSetupCopyThemes() {
	...
	jQuery( 'a.make-child-theme' ).unbind( 'click' ).click( function() {
		jQuery( '#ajax-saving' ).fadeIn( 300 );	
		jQuery( '#pane-content-pane-2 .right-area' ).animate( { opacity: 0.5 } );
...
function wptouchLoadSupportPosts() {	
	var wptouchboardSupport = jQuery( '#support-threads-box-ajax' );
...
function wptouchCheckHomePageRedirect() {
	if ( jQuery( '#enable_home_page_redirect' ).attr( 'checked' ) ) {
		...
		jQuery( '#home_page_redirect_target' ).parent().hide();	
	}	
}
	...
	if ( adminTabSwitchLinks.length ) {
		adminTabSwitchLinks.unbind( 'click' ).click( function() {
			...
			setTimeout( function() {				
				...
				jQuery( '#wptouch-tabbed-area' ).animate( { opacity: 1.0 }, 500 );
			},
			600 );
...
function WPtouchSetupLicenseArea() {		
	jQuery( 'a.wptouch-remove-license' ).unbind('click').click( function() {
	...
	var wptouchLicenseResetLink = jQuery( '#reset-licenses' );
	if ( wptouchLicenseResetLink.length ) {
		jQuery( '#reset-licenses' ).unbind( 'click' ).bind( 'click', function() {
			if ( confirm( WPtouchCustom.reset_license_text ) ) {
