<?php 
	...
	if ( wptouch_theme_supports_ipad() ) {
		if ( $settings->ipad_support === 'full' ) {  
			$ipad_support = '(' . __( 'iPad theme ON', "wptouch-pro" ) . ')'; 
		} else { 
			$ipad_support = '(' . __( 'iPad Theme OFF', "wptouch-pro" ) . ')';
		}
	}
?>
<ul>
	<li><?php _e( "Active Theme", "wptouch-pro" ); ?>: <span><?php wptouch_bloginfo( 'active_theme_friendly_name' ); ?> <?php echo $ipad_support; ?></span></li>
	<li><?php _e( "WPtouch Version", "wptouch-pro" ); ?>: <span><?php wptouch_bloginfo( 'version' ); ?></span> 
	<?php if ( wptouch_is_upgrade_available() ) { ?>
	<a id="upgrade-link" href="<?php echo admin_url(); ?>plugins.php?plugin_status=upgrade"><?php _e( "Upgrade Available", "wptouch-pro" ); ?></a></li>
	<?php } else { ?>
