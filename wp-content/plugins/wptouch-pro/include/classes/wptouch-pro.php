...
class WPtouchPro {
	...
	function initialize() {	
		...
		// Load the current theme functions.php, or the child root functions if it exists		
	...
	//	add_filter( 'init', array( &$this, 'setup_ipad' ) );
	...
	function output_child_scripts() {
		if ( file_exists( STYLESHEETPATH . '/style.css' ) ) {
			echo "<link rel='stylesheet' type='text/css' media='screen' href='" . $this->get_stylesheet_directory_uri( false ) . "/style.css'></link>\n";	
		}	
	}
	...
	function get_template_directory_uri( $directory, $template = false, $root = false ) {
		...
		if ( $this->has_parent_theme() ) {
			...
			return WP_CONTENT_URL . $parent_info->location . '/' . apply_filters( 'wptouch_parent_device_class', $this->get_active_device_class() );
		} else {
			return WP_CONTENT_URL . $theme_info->location . '/' . $this->get_active_device_class();
		}
	}
	...
	function add_static_menu_items( $menu_items ) {
		...
		if ( $settings->menu_show_rss ) {
			$bottom_items[ __( 'RSS', 'wptouch-pro' ) ] = wptouch_create_menu_item( WPTOUCH_ICON_RSS, 1, __( 'RSS', 'wptouch-pro'), 'link', false, 0, false, wptouch_get_bloginfo( 'rss_url'), 'email' );		
		}
...
//		}
	...
	function add_ignored_urls() {
		...
		if ( $ignored_urls ) {
			...
			echo "<script type='text/javascript'>var wptouch_ignored_urls = new Array(" . implode( ',', $new_urls ) . ");</script>";	
		}			
}
