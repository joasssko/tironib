...
include( WPTOUCH_DIR . '/include/classes/array-iterator.php' );
include( 'wordpress-menu.php' );
 ...
 *		\internal
 */
function wptouch_create_menu_item( $page_id, $depth, $title, $item_type, $has_children, $post_parent, $duplicate_link, $item_link, $item_class = false ) {
	global $wptouch_pro;
...
function wptouch_build_menu_tree( $parent = 0, $depth = 1, &$data, $exclude_disabled = false ) {
	...
	if ( $settings->custom_menu_name != 'none' ) {
		...
		/*
		// Handle WordPress custom menus
		$menu_items = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM wp_posts WHERE ID IN (SELECT object_id FROM wp_term_relationships WHERE term_taxonomy_id = %d) ORDER BY menu_order", $settings->custom_menu_name ) );
		
		if ( $menu_items ) {
			foreach( $menu_items as $menu_item ) {
				$menu_options = new stdClass;
								
				if ( is_numeric( $menu_item->post_name ) ) {
					
					$menu_type = get_post_meta( $menu_item->post_name, '_menu_item_type', TRUE );
					switch( $menu_type ) {
						case 'post_type':
							$object_id = get_post_meta( $menu_item->post_name, '_menu_item_object_id', TRUE );
							$post_info = get_post( $object_id );
							
							$menu_options->title = $post_info->post_title;	
							$menu_options->page_id = $post_info->ID;		
							$menu_options->item_link = $wptouch_pro->convert_to_internal_url( get_permalink( $post_info->ID ) );						
							break;
						case 'taxonomy':
							$object_id = get_post_meta( $menu_item->post_name, '_menu_item_object_id', TRUE );
							$menu_item_object = get_post_meta( $menu_item->post_name, '_menu_item_object', TRUE );
							$term = get_term_by( 'id', $object_id, $menu_item_object );	
							$menu_options->title = $term->name; 
							break;
					}
							
				} else {
					$menu_options->title = $menu_item->post_title;
					$menu_options->page_id = $menu_item->ID;	
				...
				$data[ $menu_options->title ] = $menu_options;						
			}	
		}
		*/
...
function wptouch_get_menu_icon() {
	...
	if ( isset( $settings->menu_icons[ wptouch_get_menu_id() ] ) ) {
		return WP_CONTENT_URL . $settings->menu_icons[ wptouch_get_menu_id() ];
	} else {
...
function wptouch_get_menu_item_classes() {
	...
	if ( $wptouch_menu_item->post_parent ) {
	...
	$set_info = $wptouch_pro->get_set_with_icon( str_replace( WP_CONTENT_URL, WP_CONTENT_DIR, $icon_location ) );
	...
	if ( isset( $settings->menu_icons[ $icon_type ] ) ) {
		...
		if ( $site_icons && isset( $site_icons[ $icon_type ] ) ) {	
			...
			if ( isset( $settings->menu_icons[ WPTOUCH_ICON_DEFAULT ] ) ) {
				$icon = WP_CONTENT_URL . $settings->menu_icons[ WPTOUCH_ICON_DEFAULT ];
			} else {
